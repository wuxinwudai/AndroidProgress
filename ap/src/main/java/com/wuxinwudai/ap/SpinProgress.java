...
/**
 * SpinProgress 旋转进度显示控件，不用于显示实际进度
 * @author 吾心无待 于2016年06月21日
 */
public class SpinProgress extends View {
    //region 构造函数

    /**
     * 构造函数，初始化 SpinProgress 类的一个新实例
     * @param context 上下文对象
     */
    public SpinProgress(Context context) {
        this(context, null);
    }
     ...
     */
    public SpinProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SpinProgress, defStyleAttr, 0);
        parseAttributes(ta);
        setAnimationEnabled();
    }
     ...
     * @param attrs   属性列表
     */
    public SpinProgress(Context context, AttributeSet attrs) {
    ...
    protected int mCircleRadius = 20;//圆环半径
    protected int mBarWidth = 3;//进度条宽度，sp 为单位
    protected int mRingWidth = 3;//圆环宽度,sp 位单位
    protected int mBarColor = 0xFF4089E8;//旋转条颜色
...
//        setMeasuredDimension(width, height);
    ...
    private void parseAttributes(TypedArray a) {
        ...
        mCircleRadius = (int) a.getDimension(R.styleable.SpinProgress_ProgressCircleRadius, mCircleRadius);
        mBarWidth = (int) a.getDimension(R.styleable.SpinProgress_ProgressBarWidth, mBarWidth);
        mRingWidth = (int) a.getDimension(R.styleable.SpinProgress_ProgressRingWidth, mRingWidth);
        float baseSpinSpeed =
                a.getFloat(R.styleable.SpinProgress_ProgressSpinSpeed, mSpinSpeed / 360.0f);
        mSpinSpeed = baseSpinSpeed * 360;
        mSpinCycleTime = a.getInt(R.styleable.SpinProgress_ProgressSpinCycleTime, (int) mSpinCycleTime);
        mBarColor = a.getColor(R.styleable.SpinProgress_ProgressBarColor, mBarColor);
        mRingColor = a.getColor(R.styleable.SpinProgress_ProgressRingColor, mRingColor);
        a.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        canvas.drawArc(mCircleBounds, 360, 360, false, mRingPaint);//绘制圆环
    ...
    private void updateBarLength(long deltaTimeInMilliSeconds) {
        if (mPausedTimeWithoutGrowing >= PauseGrowingTime) {
